...
type ApiRolesResp = { roles: Role[] };
type ApiStaffResp = { rows: Staff[]; total: number; page?: number; limit?: number };
type ApiUserRolesMapResp = { map: Record<string, string[]> };
...
async function fetchJSON<T>(input: RequestInfo, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      `HTTP ${res.status} ${res.statusText} — Expected JSON but got: ${text.slice(0, 140)}...`
    );
  }
...
export default function AdminSecurityPage() {
  ...
  async function loadRoles() {
    const data = await fetchJSON<ApiRolesResp>('/api/admin/security/roles');
    setRoles(data.roles || []);
  }
  ...
  async function loadStaff(p = page, keyword = q) {
    ...
    const data = await fetchJSON<ApiStaffResp>('/api/admin/security/staff?' + qs.toString());
    const fetchedRows: Staff[] = data.rows || [];
    setRows(fetchedRows);
    setTotal(data.total || 0);

    // fetch role map cho các user hiển thị
    const ids: string[] = fetchedRows.map((r) => r.user_id);
    if (ids.length) {
      const usp = new URLSearchParams();
      ids.forEach((id: string) => usp.append('user_ids', id));

      const mapResp = await fetchJSON<ApiUserRolesMapResp>(
        '/api/admin/security/user-roles?' + usp.toString()
      );
      const next: Record<string, Set<string>> = {};
      ids.forEach((id: string) => {
        const arr: string[] = (mapResp.map && mapResp.map[id]) || [];
        next[id] = new Set(arr);
      });
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border bg-white p-3 shadow">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <button onClick={() => goto(1)} className={BTN_OUTLINE}>
            Tìm
          </button>
          <button onClick={() => refetchAll()} className={BTN_OUTLINE}>
            Làm mới
          </button>
        </div>
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td className="px-3 py-6 text-slate-500" colSpan={roles.length + 1}>
                    Đang tải…
                  </td>
                </tr>
              ) : rows.length ? (
                rows.map((u) => (
                  <tr key={u.user_id} className="border-t">
                    <td className="px-3 py-2">
                      ...
                      {!u.is_active && (
                        <span className="mt-1 inline-block rounded-full bg-slate-100 px-2 py-[2px] text-[11px] text-slate-600">
                          Inactive
                        </span>
                      )}
                    </td>
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className={BTN_OUTLINE} onClick={() => goto(1)} disabled={page <= 1}>
              Đầu
            </button>
            <button className={BTN_OUTLINE} onClick={() => goto(page - 1)} disabled={page <= 1}>
              ← Trước
            </button>
            <button
            ...
            <button
              className={BTN_OUTLINE}
              onClick={() => goto(Math.max(1, Math.ceil(total / limit)))}
              disabled={page >= Math.max(1, Math.ceil(total / limit))}
            >
              Cuối
            </button>
