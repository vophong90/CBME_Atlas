...
// =====================
// UI helper classes (đồng bộ palette như trang Students)
// =====================
const BTN_PRIMARY =
...
const BTN_DANGER =
  'px-3 py-1.5 rounded-lg font-semibold bg-red-600 text-white hover:bg-red-700 disabled:bg-gray-300 disabled:cursor-not-allowed';
...
export default function AdminUsersPage() {
  ...
  // =====================
  // Fetch danh sách thật từ API (schema public.staff)
  // =====================
  useEffect(() => {
    (async () => {
      try {
  ...
  async function onImportConfirm() {
    ...
    try {
      ...
      const ref = await fetch('/api/admin/users/list', { cache: 'no-store' });
      const js = await ref.json();
                  ...
                  <tr className="border-t bg-slate-50">
                    <td className="px-3 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNewRow((s) => ({ ...s, full_name: e.target.value }))
                        }
                        placeholder="Họ tên"
                        className={INPUT_SM}
                      />
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-end gap-2">
                        <input
                          ...
                          className="w-48 border rounded-lg px-2 py-1 text-sm outline-none focus:border-brand-300 focus:ring-2 focus:ring-brand-300"
                        />
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
          <div className="w-full max-w-2xl rounded-xl border bg-white p-4 shadow">
            ...
            <p className="text-sm text-slate-600">
              Chọn tệp .csv với header: <code>email,full_name,department_code,roles,password</code>
            </p>
