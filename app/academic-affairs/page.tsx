...
type Framework = {
  id: string;                 // id của curriculum_frameworks
  doi_tuong: string;
  chuyen_nganh: string;
  nien_khoa: string;
  created_at: string;
...
> = {
  plo: {
    title: 'Tải PLO (CSV)',
    helper: '2 cột (không header): code,description',
  },
  pi: {
    title: 'Tải PI (CSV)',
    helper: '2 cột (không header): code,description',
  ...
  plo_pi: {
    ...
    helper: '2 cột (không header): plo_code,pi_code',
  },
  plo_clo: {
    title: 'Liên kết PLO–CLO (CSV)',
    helper: '4 cột (không header): plo_code,course_code,clo_code,level',
  },
  pi_clo: {
    title: 'Liên kết PI–CLO (CSV)',
    helper: '4 cột (không header): pi_code,course_code,clo_code,level',
  },
};
...
export default function AcademicAffairsPage() {
  ...
  // Tabs
  const [activeTab, setActiveTab] = useState<'framework' | 'students'>('framework');
  ...
  async function fetchFrameworks() {
    // API này cần trả về danh sách từ bảng curriculum_frameworks
  ...
  async function doUpload(kind: UploadKind) {
    ...
    fd.append('framework_id', selectedId); // <- framework hiện đang chọn (curriculum_frameworks.id)
    fd.append('kind', kind);
    fd.append('file', file);
  ...
  // List existing (optional, để kiểm tra dữ liệu đã insert từ CSV)
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 space-y-6">
      ...
      {activeTab === 'students' && (
        <section className="rounded-xl border bg-white p-5 shadow space-y-6">
          <h2 className="text-lg font-semibold">Tạo tài khoản sinh viên</h2>
