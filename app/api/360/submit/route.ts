...
export async function POST(req: Request) {
  ...
  // 1) Lấy request (RLS: người chấm)
  const { data: reqRow, error } = await supabase
    .from('evaluation_requests')
    .select(`
      id, status, group_code, campaign:campaign_id ( id, rubric_id ), evaluatee:evaluatee_user_id ( id )
    `)
    .eq('id', request_id)
    .single();
  ...
  // 2) Tạo observation (kind = eval360), teacher_user_id tạm dùng làm "người chấm" chung
  const { data: obs, error: oerr } = await supabase
    .from('observations')
    .insert({
      rubric_id: reqRow.campaign.rubric_id,
      student_user_id: reqRow.evaluatee.id,
  ...
  // 4) Đánh dấu request submitted + gắn observation_id (người chấm có quyền update RLS)
  ...
  // 5) Cập nhật student_clo_results bằng RPC (service role)
