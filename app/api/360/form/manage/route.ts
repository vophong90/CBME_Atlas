...
import { createServerClient } from "@supabase/ssr"; // ⬅️ dùng @supabase/ssr
import { createServiceClient } from "@/lib/supabaseServer";
import { ensureQA } from "@/lib/perm360";

async function getUserFromCookies() {
  // ⬅️ Next 15: cookies() là Promise → cần await
...
export async function GET(req: Request) {
  try {
    ...
    let q = sb
      .from("eval360_forms")
      .select(
        "id,title,group_code,rubric_id,framework_id,course_code,status,updated_at"
      )
