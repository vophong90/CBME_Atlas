...
import { ProgressRow, DetailModal, CourseCLO, Achieve, PLO } from '../_shared';

type PLOProgressItem = { plo: PLO; items: Array<CourseCLO & { status: Achieve }> };

export default function StudentPLOPage() {
  const { studentId } = useStudentCtx();
  const url = `/api/student/plo-progress${studentId ? `?student_id=${studentId}` : ''}`;
  const { data, loading, error } = useCachedJson<PLOProgressItem[]>(url, 60_000);

  const [ploFilter, setPloFilter] = useState<'all'|'achieved'|'not_yet'|'not_learned'>('all');
  const [active, setActive] = useState<PLOProgressItem | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    const ratioFrom = (items: any[]) => {
      ...
      const got = items.filter((it:any)=>it.status==='achieved').reduce((s,it)=>s+w(Number(it.level)),0);
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={ploFilter} onChange={e=>setPloFilter(e.target.value as any)} className="border rounded-md px-2 py-1 text-sm">
      ...
      {!loading && !error && (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map(row=>(
            <ProgressRow key={row.plo.code} code={row.plo.code} title={row.plo.description} items={row.items} onClick={()=>setActive(row)} />
          ))}
          {filtered.length===0 && <div className="text-sm text-slate-500">Không có dữ liệu.</div>}
        </div>
      )}

      <DetailModal kind="PLO" open={!!active} onClose={()=>setActive(null)}
                   title={active?.plo.code || ''} desc={active?.plo.description || ''} items={active?.items || []}/>
    </section>
  );
}
